...
contract TimeRangeSupport {
    /// interface-id for supportInterface-check
     bytes4 internal constant ID = 0x51533fb7;

    /// returns the minimum and maximun time this device can be rented
    /// @param id the device id
    /// @return min and max time for renting
    function getRangeSeconds(bytes32 id) external constant returns (uint32 min, uint32 max);
    ...
    function setRangeSeconds(bytes32 id, uint32 min, uint32 max) external;
}
